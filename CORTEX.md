...
> Meridian 多智能体协作体系的通用角色分类框架

## 定位

Meridian（灵枢/经络）是信息传递与流动的通路，CORTEX（皮层）是认知加工与决策的中枢。两者合在一起构成完整的数字神经系统——Meridian 定义协作的骨架，CORTEX 定义每个节点上的智能。
...
**选择理由**：
1. **与 Meridian 形成完整的神经系统隐喻**——Meridian（灵枢/经络）是信息传递与流动的通路，Cortex（皮层）是认知加工与决策的中枢。经络负责传导，皮层负责思考。
...
本框架由 Meridian agent team 通过多轮多模型讨论（mux_broadcast）协作设计：
